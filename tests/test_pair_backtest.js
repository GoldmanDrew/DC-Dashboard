...
test("flat prices: only borrow drag on short leg", () => {
  const rows = [];
  ...
  const out = simulateInversePairBacktest(rows, {
    ...
    slippageBps: 0,
    /** Canonical: negative = fee (10% p.a. drag to short). */
    avgBorrowAnnual: -0.1,
  });
...
test("computePairBacktestRiskSeries: CAGR finite only after min span", () => {
  ...
  /** Enough consecutive calendar days for ≥63 chart rows and ≥0.25y between first/last risk dates. */
